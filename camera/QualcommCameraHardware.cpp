...
{
    ALOGI("native_set_parms %d %d %d",type,length,*(int *)value);
...
// TODO do properly
...
#endif
        {
           ...
           ALOGV("unregistering all preview buffers");
...
#endif
             type = MSM_PMEM_VIDEO;
             ALOGV("%s: unregister record buffers[%d] with camera driver", __FUNCTION__, cnt);
             if(recordframes) {
               register_buf(mRecordFrameSize,
                  mRecordFrameSize, CbCrOffset, 0,
                  recordframes[cnt].fd,
...
bool QualcommCameraHardware::initRaw(bool initJpegHeap)
{
    ...
    */
    ...
    ALOGE("w=%d h=%d offset=%d",mPictureWidth,mPictureHeight,mCbCrOffsetRaw);
    cam_buf_info_t buf_info;
    ...
    ALOGE("new w=%d h=%d offset=%d yoffset=%d",mPictureWidth,mPictureHeight,mCbCrOffsetRaw,mYOffset);
    int mBufferSize;
    int CbCrOffset;
    ...
    ALOGE("poat w=%d h=%d offset=%d",mPostviewWidth,mPostviewHeight,CbCrOffset);
...
status_t QualcommCameraHardware::getBuffersAndStartPreview() {
	...
	//yyan todo use handle to find out mPreviewBuffer
			...
			if(retVal == NO_ERROR) {
                ...
                //ALOGV("Handle %p, Fd passed:%d, Base:%p, Size %p",
                ...
                if(handle) {
                  ...
                  //ALOGV("Got the following from get_mem data: %p, handle :%d, release : %p, size: %d",
                  //     mPreviewMapped[cnt]->data,
                  //     mPreviewMapped[cnt]->handle,
                  //     mPreviewMapped[cnt]->release,
                  //     mPreviewMapped[cnt]->size);
                  ALOGV(" getbuffersandrestartpreview deQ %d", handle->fd);
                  ...
                  ALOGV("Registering buffer %d with fd %d with kernel",cnt,handle->fd);
                  register_buf(mBufferSize,
 ...
 // Dequeue Thumbnail/Postview  Buffers here , Consider ZSL/Multishot cases
        for (cnt = 0; cnt < (mZslEnable? (MAX_SNAPSHOT_BUFFERS-2) : numCapture); cnt++) {
            ...
            ALOGV(" : dequeing thumbnail buffer fd %d", handle->fd);
            if(retVal != NO_ERROR) {
 ...
 //   if(!mPreviewInitialized && !mCameraRunning) {   // TODO just for testing
        ALOGV("setPreviewWindow: Starting preview after buffer allocation");
        startPreviewInternal();
 //   }
    ALOGV(" %s : X ",__FUNCTION__);
    return NO_ERROR;
...
void QualcommCameraHardware::release()
{
    ...
    deinitRawSnapshot();
    ALOGV("release: clearing resources done.");
    if(mCurrentTarget == TARGET_MSM8660) {
...
bool QualcommCameraHardware::initRecord()
{
    ...
    ALOGE("%s  Record fd is %d ", __func__, mRecordfd[cnt]);
        ...
        ALOGV("Registering buffer %d with kernel",cnt);
                  register_buf(mRecordFrameSize,
                             mRecordFrameSize, CbCrOffset, 0,
                             recordframes[cnt].fd,
                             0,
...
status_t QualcommCameraHardware::setRecordSize(const CameraParameters& params)
{
    ...
    ALOGE("debug: %s: preview dimensions: %dx%d", __FUNCTION__, previewWidth, previewHeight);
    ALOGE("debug: %s: video dimensions: %dx%d", __FUNCTION__, videoWidth, videoHeight);
    mDimension.display_width = previewWidth;
    mDimension.display_height= previewHeight;
    return NO_ERROR;
}
...
status_t QualcommCameraHardware::setPreviewSize(const CameraParameters& params)
{
    ...
    ALOGE("debug: requested preview size %d x %d", width, height);
...
bool QualcommCameraHardware::updatePictureDimension(const CameraParameters& params, int& width, int& height)
{
    ...
    ALOGE("debug: updatePictureDimension: %dx%d <- %dx%d", width, height,
...
status_t QualcommCameraHardware::setPictureSize(const CameraParameters& params)
{
    ...
    ALOGE("debug: requested picture size %d x %d", width, height);
    ...
    if (str != NULL) {
        ...
        if (value != NOT_FOUND) {
            ...
            bool ret = native_set_parms(CAMERA_PARM_EXPOSURE, sizeof(value),
                                       (void *)&value);
//         return ret ? NO_ERROR : UNKNOWN_ERROR; //shutt1e hack for stock jb camera
	    return NO_ERROR;
        }
    }
...
// return BAD_VALUE;  //shutt1e hack for stock jb camera
return NO_ERROR;
}
...
void QualcommCameraHardware::getCameraInfo()
{
    ...
    *(void **)&LINK_mm_camera_get_camera_info =
    ...
    ALOGI("getCameraInfo: Number Of Cameras = %d", HAL_numOfCameras);
    for(int i = 0; i < HAL_numOfCameras; i++) {
        ...
        ALOGI("modes_supported: %d", HAL_cameraInfo[i].modes_supported);
